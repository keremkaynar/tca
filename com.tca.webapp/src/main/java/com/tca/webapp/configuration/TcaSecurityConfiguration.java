...
public class TcaSecurityConfiguration extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    http.authorizeRequests()
        .antMatchers("/showcontacts").hasAnyRole("USER", "ADMIN")
        .antMatchers("/showcontactdetails").hasRole("ADMIN")
        .and().formLogin().loginPage("/login").defaultSuccessUrl("/showcontacts")
        .and().csrf()
